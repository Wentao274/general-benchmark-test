...
| 模型名称 | 推理框架 | 输入长度 | 输出长度 | 并发数 | 输入token吞吐量\_\<芯片类型\> (toks/s) | 输出token吞吐量\_\<芯片类型\> (toks/s) | 总token吞吐量\_\<芯片类型\> (toks/s) | Mean TTFT\_\<芯片类型\> (ms) | P99 TTFT\_\<芯片类型\> (ms) | Mean TPOT\_\<芯片类型\> (ms) |
|---|---|---|---|---|---|---|---|---|---|---|
| \<模型名\> | \<框架\> | 2048 | 512 | 1 | | | | | | |
| \<模型名\> | \<框架\> | 2048 | 512 | 4 | | | | | | |
| \<模型名\> | \<框架\> | 2048 | 512 | 8 | | | | | | |
| ... | ... | ... | ... | ... | | | | | | |
| \<模型名\> | \<框架\> | 65536 | 1024 | 128 | | | | | | |
...
./bench.sh -F sglang -f -u http://127.0.0.1:8080 -m /data/model -n model-name
./bench.sh -F vllm   -f -u http://127.0.0.1:8000 -m /path -n name

# 后台执行（默认）
./bench.sh -F sglang -u http://127.0.0.1:8080 -m /data/model -n model-name
# 输出示例：
#   SGLang benchmark 已在后台启动 (PID: 12345)
#   日志文件: ./sglang_reports/sglang_bench_20260827_143022.log
#   监控进度: tail -f "./sglang_reports/sglang_bench_20260827_143022.log"
```
...
```
模型名称,推理框架,输入长度,输出长度,并发数,输入token吞吐量_<芯片类型> (toks/s),输出token吞吐量_<芯片类型> (toks/s),总token吞吐量_<芯片类型> (toks/s),Mean TTFT_<芯片类型> (ms),P99 TTFT_<芯片类型> (ms),Mean TPOT_<芯片类型> (ms)
```
...
# 默认后台执行，加 -f 前台
./prefill_bench.sh -F sglang -f -u http://... -m /path -n name

# 自定义输入长度和并发
./prefill_bench.sh -F sglang -c 1,8,64 -i "32768 1,65536 1"
```
...
| 模型名称 | 输入长度 | 输出长度 | 并发数 | prefill吞吐量\_\<芯片类型\> (toks/s) | Mean TTFT\_\<芯片类型\> (ms) |
|---|---|---|---|---|---|
| \<模型名\> | 65536 | 1 | 1 | | |
| \<模型名\> | 65536 | 1 | 4 | | |
| ... | ... | ... | ... | | |
...
# 默认后台执行，加 -f 前台
./decode_bench.sh -F sglang -f -u http://... -m /path -n name

# 自定义前缀、输出和并发
./decode_bench.sh -F sglang -p 8192,65536 -o 512 -c 1,16,128
```
...
| 模型名称 | 前缀长度 | 输出长度 | 并发数 | decode吞吐量\_\<芯片类型\> (toks/s) | Mean TPOT\_\<芯片类型\> (ms) | Mean TTFT\_\<芯片类型\> (ms) |
|---|---|---|---|---|---|---|
| \<模型名\> | 4096 | 1024 | 1 | | | |
| \<模型名\> | 4096 | 1024 | 4 | | | |
| ... | ... | ... | ... | | | |
